<?php
    ...
    if(isset($_POST['medium'])){
        $_SESSION['medium2'] = $_POST['medium'];
        header("Location:image_standard.php");
        return;
    }
?>
...
<h3 class="alert alert-primary" role="alert">Select Your standard:</h3>
        ...
        <?php 
            $stmt = $pdo->query('SELECT DISTINCT std FROM que_ans WHERE medium="'.$_SESSION['medium2'].'"');
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

            foreach($rows as $row )
            {?>
                <form action="./image_subject.php" method="POST">
