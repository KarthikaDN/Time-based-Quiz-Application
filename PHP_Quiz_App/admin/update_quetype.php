<?php
    ...
    if(isset($_POST['chapname'])){
        ...
        header("Location:update_quetype.php");
        return;
    }
?>
...
<h3 class="alert alert-primary" role="alert">Choose question type</h3>
        ...
        <?php 
            $stmt = $pdo->query('SELECT DISTINCT quetype FROM que_ans WHERE medium="'.$_SESSION['medium'].'" AND std="'.$_SESSION['std'].'" AND subject="'.$_SESSION['subject'].'" AND chapname="'.$_SESSION['chapname'].'"');
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

            foreach($rows as $row )
            {?>
                <form action="./update_start.php" method="POST">
