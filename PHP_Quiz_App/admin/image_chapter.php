<?php
    ...
    if(isset($_POST['subject'])){
        ...
        header("Location:image_chapter.php");
        return;
    }
?>
...
<h3 class="alert alert-primary" role="alert">Select Your Chapter</h3>
        ...
        <?php 
            $stmt = $pdo->query('SELECT DISTINCT chapname FROM que_ans WHERE medium="'.$_SESSION['medium2'].'" AND std="'.$_SESSION['std2'].'" AND subject="'.$_SESSION['subject2'].'"');
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

            foreach($rows as $row )
            {?>
                <form action="./image_quetype.php" method="POST">
